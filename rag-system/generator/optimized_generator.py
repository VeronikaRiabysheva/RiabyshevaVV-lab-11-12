...
class OptimizedLLMGenerator:
    def __init__(self, model_name: str = "gpt2-medium"):
        ...
        self.retriever = VectorStore(collection_name="rag_documents")  # Создаём экземпляр VectorStore
        self._initialize_database()  # Вызываем метод для заполнения БД

    def _initialize_database(self):
        """Инициализация и заполнение базы данных документами"""
    ...
    def generate_optimized_response(self, query: str, context: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(generated_text) <= len(prompt):
                answer = "Модель не сгенерировала новый текст (возможно, промпт завершён)."
            else:
...
if __name__ == "__main__":
    ...
    context = [{"content": "What is ai?", "metadata": {"title": "Test", "category": "AI"}, "similarity_score": 0.9}]
